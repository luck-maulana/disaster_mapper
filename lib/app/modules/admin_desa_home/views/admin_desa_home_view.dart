...
import 'detail_pendataan1.dart';

class AdminDesaHomeView extends GetView<AdminDesaHomeController> {
  final AuthController authController = Get.find<AuthController>();
  String? admindesaSelectedDesa;
  AdminDesaHomeView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    String? admindesaSelectedDesa = authController.admindesaSelectedDesa;

    controller.fetchPendataanByDesa(admindesaSelectedDesa!);
  ...
  void performSearch() {
    ...
    if (query.isNotEmpty) {
      List<PendataanData> searchData =
          controller.pendataanList.where((admindesa) {
        return admindesa.nik.contains(query) ||
            admindesa.nomorKK.contains(query) ||
            admindesa.namaPemilik.contains(query) ||
            admindesa.tingkatKerusakan.contains(query);
      }).toList();
